...
@Component({
  selector: "app-admin-student-enrollment",
  templateUrl: "./admin-student-enrollment.component.html",
  styleUrls: ["./admin-student-enrollment.component.css"],
  providers: [MessageService, ConfirmationService],
})
export class AdminStudentEnrollmentComponent implements OnInit {
  ...
  ngOnInit() {
    this.receivedData = history.state.studentData;

    this.addClass = false;
    this.selectedCategories1 = this.categories1.slice(2, 4);
    this.showGoBack = true;
    this.extraTabRequiredVal = this.receivedData[3];
    this.extraTabRequiredValues = this.receivedData[4];
    this.eventId = this.receivedData[0];
    this.eventYear = this.receivedData[1];

    this.schoolId = this.receivedData[2];
  ...
  setSubGame(gameName, gameType, gameId, ageRange, gender, category) {
    ...
    this.studentEnrollmentService
      .getSubGameData(this.gameId, ageRange, gender, "admin")
      .subscribe(
        (response) => {
          if (response !== "") {
  ...
  onSubmit() {
    this.submitted = true;
    const formData = new FormData();
    const schoolId = this.receivedData[2];
    let subGameId;
    let subGameName;
    let govtIdAadhar;
    let govtIdPassport;
