...
export class PaymentInvoiceService {
  ...
  downloadInvoicePdf(yearVal, reportval, schoolID) {
    let str =
      "paymentInvoice/downloadInvoice/" +
      yearVal +
      "/" +
      reportval +
      "/" +
  ...
  downloadReceiptPdf(yearVal, reportval, schoolID) {
    let str =
      "paymentInvoice/downloadReceipt/" +
      yearVal +
      "/" +
      reportval +
      "/" +
  ...
  saveStudentAttendance(studentInfo) {
    return this.http
      .post<any>(this.serverUrl + "paymentInvoice/saveAttendance/", studentInfo)
      .pipe(catchError(this.handleError));
  }

  getStudentData(evenId, gameId, gameType, paymentStatusVal) {
    let str =
      "payment/getStudentData/" +
      evenId +
      "/" +
      gameId +
      "/" +
  ...
  gameSummaryDataForBothTypeGame(eventValue, gameVal) {
    let str =
      "payment/gameSummaryDataForBothTypeGame/" + eventValue + "/" + gameVal;
    return this.http
      .get(this.serverUrl + str)
      .pipe(catchError(this.handleError));
  }
